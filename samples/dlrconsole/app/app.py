...
#Verify that the APIs we depend on are there (DLR hosting APIs, Silverlight APIs)
# TODO: this should only be run in test mode
selfTest = False
...
scriptEnv = ScriptRuntime(DynamicApplication.Current.Runtime.Setup)
            
languages = ['Python', 'Ruby'] # TODO: add JScript back
...
r.MouseEnter += function (s, e) {
  s.Foreground = new wpf.SolidColorBrush(wpf.Colors.White);
}

r.MouseLeave += function (s, e) {
  s.Foreground = new wpf.SolidColorBrush(wpf.Colors.Black);
}"""
...
canvas.background = SolidColorBrush.new Colors.blue
...
r.mouse_enter do |s,e|
    s.foreground = SolidColorBrush.new Colors.white
end
r.mouse_leave do |s,e|
    s.foreground = SolidColorBrush.new Colors.black
end"""
...
class LanguageEngine:
    ...
    def TryExpression(self, text):            
        ...
        if (props == ScriptCodeParseResult.Complete) or (props == ScriptCodeParseResult.Empty):
            result = text
        else:
            result = None
...
class RubyEngineHelper(LanguageEngine):
    def __init__(self, *args):
        ...
        self.engine.CreateScriptSourceFromString('''
        require 'IronPython'
        require 'wpf.rb'
        ...
        ''', SourceCodeKind.File).Compile().Execute(scriptEnv.CreateScope())
    
    # Format the reslt using Ruby's inspect & not Python's repr
    def FormatResult(self, obj):
        clr.AddReference('IronRuby', 'IronRuby.Libraries')
        import IronRuby
        IronRuby.Ruby.GetExecutionContext(self.engine).DefineGlobalVariable('last_result', obj)
        result = self.engine.CreateScriptSourceFromString('$last_result.inspect.to_clr_string').Execute(scriptEnv.CreateScope()) 
        return result
...
class DLRConsoleApp:
    ...
    def ExecuteCode(self):
        ...
        self.ReloadXaml()
        try:            
            self.codeEditor.Engine.CreateScriptSourceFromString(text, SourceCodeKind.File).Compile().Execute(self.console.input.CurrentScope)
        except Exception, e:
    ...
    def RedirectOutput(self, console):
        sys.stdout = console.console
        sys.stderr = console.console
        console.CurrentScope.SetVariable("console", console.console)
        console.Engine.CreateScriptSourceFromString('''
import sys
sys.stdout = console
sys.stderr = console
        ''', SourceCodeKind.File).Execute(console.CurrentScope)
        
    def PredefineVariables(self, console, canvas):
    ...
    def StartConsole(self, console, canvas, langInfo):
        self.RedirectOutput(console)
        self.PredefineVariables(console, canvas)
    ...
    def TestColorize(self, console):
        ...
        tc.Initialize(None, su, SourceLocation(0,1,1))
...
class TextInputHandler:
    def __init__(self, root):
        ...
        keys[78] = '*', '*'
        keys[79] = '=', '+' # Firefox 3 on Windows (Macbook?) needs this for '='
        keys[80] = '-', '_' # Firefox 2-3on Windows (Macbook?) needs this for '_'
        keys[81] = '.', '.'
        keys[82] = '/', '/'
...
class Editor(TextBuffer):
    ...
    def SetLanguage(self, name):
        ...
        self.TokenCategorizer = self.Engine.GetService[TokenCategorizer]()
...
class ConsoleInput(Editor):
    ...
    def DoCompletions(self):
        self.AddInput('.')
        if self.le.longName != "Ruby": # TODO: implement completions for Ruby
    ...
    def Execute(self, text):
        self.Engine.CreateScriptSourceFromString(text).Compile().Execute(self.CurrentScope)
        
    def HandleException(self, e):
        dfs = Microsoft.Scripting.Runtime.RuntimeHelpers.GetDynamicStackFrames(e)
    ...
    def DoSingleLine(self, text, forceExecute):        
        ...
        if text1:
            ...
            try:
                ret = self.Engine.CreateScriptSourceFromString(text).Compile().Execute(self.CurrentScope)
                if ret is not None:
    ...
    def DoMultiLine(self, text, forceExecute):
        ...
        if forceExecute or self.IsComplete(text, allowIncomplete):
            ...
            try:
                self.Engine.CreateScriptSourceFromString(text, SourceCodeKind.InteractiveCode).Compile().Execute(self.CurrentScope)
            except Exception, e:
                self.HandleException(e)
    ...
    def IsComplete(self, text, allowIncomplete):
        SCP = ScriptCodeParseResult
        props = self.Engine.CreateScriptSourceFromString(text, SourceCodeKind.InteractiveCode).GetCodeProperties()
        result = (props != SCP.Invalid) and (props != SCP.IncompleteToken) and (allowIncomplete or (props != SCP.IncompleteStatement))
...
class CodeLine(SimpleLine):
    ...
    def Colorize(self):
        ...
        tc.Initialize(state, su, SourceLocation(0,1,1))
        try:
            tokens = tc.ReadTokens(len(code))
        except:
...
def GetTokenTrigger(console, code):
    ...
    token_categorizer.Initialize(None, source_unit, SourceLocation(0,1,1))
...
def FindExpression(text, engine):
    ...
    token_categorizer = engine.GetService[TokenCategorizer]()
    token_categorizer.Initialize(None, source_unit, SourceLocation(0,1,1))
...
if selfTest:
    # test DLR hosting APIs
    # create a dummy DLR console, and feed it some input
    global root
    root = Canvas()
    dummyApp = DLRConsoleApp(root)
    console = dummyApp.console.input
    ...
    # test completion code path
    console.AddInput("canvas")
    class DummyKey: pass
    k = DummyKey()
    k.Text = '.'
    console.HandleKey(k)
